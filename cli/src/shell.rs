...
pub fn ensure_shell(cfg: &IexConfig) {
  ...
  "exclude": ["node_modules"]
    ...
    if ios_dir.exists() {
        ...
        patch_podfile_disable_prebuilt(&ios_dir.join("Podfile"));
...
fn ensure_entry_points(shell: &Path) {
    ...
    let index_macos = shell.join("index.macos.js");
    if !index_macos.exists() {
        fs::write(
            &index_macos,
            include_str!("templates/index.macos.template.js"),
        ).unwrap();
    }
...
fn patch_podfile_disable_prebuilt(podfile: &Path) {
    let Ok(content) = fs::read_to_string(podfile) else { return };
    if content.contains("RCT_USE_PREBUILT_RNCORE") { return; }
    let prelude = "# Disable prebuilt React-Core: it strips C++ debug symbols that third-party\n\
                   # Fabric pods (e.g. react-native-safe-area-context) need at link time in Debug.\n\
                   ENV['RCT_USE_PREBUILT_RNCORE'] = '0'\n\n";
    let _ = fs::write(podfile, format!("{prelude}{content}"));
}
